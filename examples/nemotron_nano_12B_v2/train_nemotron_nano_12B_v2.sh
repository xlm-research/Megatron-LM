...
# ===== 基础环境与路径（按需修改） =====
WORKDIR="/data/yangbo1/Megatron-LM"
cd "$WORKDIR"

DATA_PATH="/data/yangbo1/datasets/Nemotron-Pretraining-Dataset-sample/Nemotron-CC-High-Quality_text_document"
TOKENIZER_PATH="/data/yangbo1/models/nv-community/NVIDIA-Nemotron-Nano-12B-v2-Base-hf2megatron/iter_0000000/tokenizer"  # 目录内应包含 tokenizer.model 或兼容文件
...
MODE_CONFIG=(
    ...
    --attention-backend auto # eager|flash|fa2(=fused)|auto(默认)
    --disable-bias-linear
    --hidden-dropout 0.0
...
CUDAGRAPH_CONFIG=(
    --cuda-graph-impl local          # 或 transformer_engine
    --cuda-graph-scope full  # 或 full / attn
    --cuda-graph-warmup-steps 3
    --no-check-for-nan-in-loss-and-grad
    # 可选：--use-legacy-static-engine
)
...
CMD=(
#   nsys profile -o trace_fp4 -t cuda,nvtx,cudnn,cublas --capture-range=cudaProfilerApi --capture-range-end=stop
  torchrun
    --nnodes ${NNODES}
